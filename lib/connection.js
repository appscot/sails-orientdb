...
module.exports = (function () {
  ...
  DbHelper.prototype.destroy = function(collection, options, cb) {
    var collectionInstance = this.collections[collection];
    var klassToDelete = utils.isJunctionTableThrough(collectionInstance) ? 'EDGE' : 'VERTEX';
    var self = this;
    cb = cb || _.noop;
    ...
    self.find(collection, options, function(err, results){
      ...
      self.db.delete(klassToDelete)
        .where('@rid in [' + rids.join(',') + ']')
        .one()
        .then(function (count) {
          var countInt = parseInt(count);
          if(countInt !== rids.length){
            var msg = 'deleted count [' + count + '] does not match rids length [' + rids.length +
          ...
          log.debug('# deleted records: ' + count);
          log.debug('# deleted records: ', results);
          cb(null, results);
        })
        .error(cb);
    });
