    ...
    */
    ...
    dbHelper.prototype.join = function (collection, options, cb) {
		...
		if(_query.criteria && _query.criteria.joins){
		  var fetchString = '@this.toJSON(\'fetchPlan:';
          _.forEach(_query.criteria.joins, function(fecthCriteria){
          	fetchString += fecthCriteria.parentKey + ':1';
          });
          fetchString += '\')';
          console.log('fetchPlan: ' + fetchString);
          query = this.db.select(fetchString).from(collection);
		} else {
       	 query = this.db.select().from(collection);
		}
