...
module.exports = (function () {
    ...
    */
    ...
    dbHelper.prototype.update = function (collection, options, values, cb) {
		...
		async.auto({
			createNestedAssociations: function(callback){
				if(!_document.nestedAssociations){
					callback(null, null);
				    return; 
				}
				
				// create embedded records
				async.reduce(Object.keys(_document.nestedAssociations), {},
				  function(accumulator, key, next){
					var value = _document.nestedAssociations[key];
					var data = value.values;
					
					if(typeof data !== 'array'){
						self.create(value.collection, data, function(err, klass){
						...
						async.map(data, 
							function(item, complete){
							  self.create(value.collection, item, function(err, klass){
			...
			updateForeignKeys: ['createNestedAssociations', function(callback, results){
				console.log('createNestedAssociations: ' + require('util').inspect(results.createNestedAssociations));
				_document.setForeignKeys(results.createNestedAssociations);
				callback(null, results);
			}],
			
			executeUpdate: ['updateForeignKeys', function(callback, results){
		        var query = self.db
		            .update(collection)
		        ...
		        query.all()
		            .then(function (res) {
		            	callback(null, utils.rewriteIds(res, attributes));
		            }).error(function (err) {
		                callback(err);
		            });
			}]
		}, function(err, results){
			cb(err, results.executeUpdate);
		});
    };
    ...
    dbHelper.prototype.join = function (collection, options, cb) {
		...
		if(_query.criteria && _query.criteria.joins){
          _.forEach(_query.criteria.joins, function(fetchCriteria){
          	...
          	console.log('traverse: ' + fetchCriteria.parentKey);
          });
		} else {
       	 query = query.select();
		}
        ...
        // optimistically copy params from 'from' to 'query'
        var key;
        ...
        query
            ...
            .then(function (res) {
            	...
            	var rowMap = _.reduce(cleanRes, function(accumulator, record){
            	...
            	var results = _.filter(cleanRes, function(record){
