...
module.exports = (function () {
  ...
  dbHelper.prototype.find = function(collection, options, cb) {
    ...
    try {
      query = sequel.find(collection, _query.criteria);
    } catch(e) {
      console.log('\nERROR e: ' + require('util').inspect(e));
      return cb(e);
    }
    
    var theQuery = query.query[0].replace(collection.toUpperCase(), collection);
  ...
  dbHelper.prototype.update = function(collection, options, values, cb) {
    ...
    var sequel = new Sequel(schema, sqlOptions);
    try {
      var _where = sequel.simpleWhere(collection, _query.criteria, sqlOptions);
