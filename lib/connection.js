...
module.exports = (function () {
  ...
  dbHelper.prototype.find = function(collection, options, cb) {
    ...
    if(_options.where && utils.object.hasOwnProperty(_options.where, 'id')){
      // Not sure why we are getting RID objects here
      if(!_.isArray(_options.where.id)){
        _options.where['@rid'] = _.isObject(_options.where.id) ? '#' + _options.where.id.cluster + ':' + _options.where.id.position : _options.where.id;
      } else {
        _options.where['@rid'] = [];
        _options.where.id.forEach(function(rid){
          var value = _.isObject(rid) ? '#' + rid.cluster + ':' + rid.position : rid;
          _options.where['@rid'].push(value);
        });
      }
      delete _options.where.id;
    }
    ...
    if (edge) {
      ...
      _options.where.out = edge.from;
      _options.where.in = edge.to;
      edge.keys.forEach(function(refKey) { delete _options.where[refKey]; });
    }
    ...
    try {
      _query = sequel.find(collection, _options);
    } catch(e) {
      console.log('\nERROR e: ' + require('util').inspect(e));
      return cb(e);
    }
    
    var theQuery = _query.query[0].replace(collection.toUpperCase(), collection);
    ...
    this.db
      ...
      .error(function (err) {
        console.log('\nERROR err: ' + require('util').inspect(err));
        cb(err);
      });
  };
  
  
  dbHelper.prototype.findOld = function(collection, options, cb) {
    var _query,
    ...
    if (edge) {
        // Replace foreign keys with from to
      _query.criteria.where[0].out = edge.from;
      _query.criteria.where[0].in = edge.to;
        edge.keys.forEach(function(refKey) {
          delete _query.criteria.where[0][refKey];
        });
    }

    var query;
    if (options.fetchPlan) {
      query = this.db.select("@this.toJSON('rid,fetchPlan:" + options.fetchPlan + "')");
    } else {
