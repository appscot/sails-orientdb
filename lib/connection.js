    ...
    var createAssociations = function (collectionName, targetData, callback) {
        ...
        async.eachSeries(_.keys(collection.attributes), function (key, next) {
            ...
            if (srcAttr && srcAttr.edge) {
            	...
            	// simple one-to-one case defined using { attribute: { model: 'destModel' } }
            	me.createEdge(targetData['@rid'], targetData[key], null, function () {
                    next();
                });
            } else next();
    ...
    */
    dbHelper.prototype.create = function (collection, options, cb) {
        ...
        this.db.insert()
            ...
            .one()
            .then(function (res) {
            	var data = _.extend({}, options, res);
                createAssociations.call(me, collection, data, function () {
                	cb(null, utils.rewriteIds(res, me.collections[collection].waterline.schema[collection].attributes));
                });
            }).error(function (err) {
                cb(err);
            });
    ...
    */
    dbHelper.prototype.update = function (collection, options, values, cb) {
        ...
        this.db
            ...
            .transform(transformers)
            .return('AFTER')
            .where(_query.criteria && _query.criteria.where)
            .all()
            .then(function (res) {
                cb(null, utils.rewriteIds(res, me.collections[collection].waterline.schema[collection].attributes));
            }).error(function (err) {
