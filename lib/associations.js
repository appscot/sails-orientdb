  ...
  self.connection.find(collectionName, options, function(err, results){
    ...
    results.forEach(function(record){
      ...
      criteria.joins.forEach(function(join){
        ...
        record[join.alias] = utils.rewriteIds(record[join.alias], childTableSchema, false);

        // record[join.alias].forEach(function(associatedRecord){
          // utils.cleanOrientAttributes(associatedRecord, childTableSchema); // we really should improve the fetchPlan query
        // });
      });
      
      // utils.cleanOrientAttributes(record, parentSchema); // we really should improve the fetchPlan query
