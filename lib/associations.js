...
Associations.prototype.getEdge = function getEdge(collectionName, values) {
  ...
  Object.keys(values).forEach(function(key) {
    ...
    Object.keys(referencedSchema).forEach(function(referencedSchemaKey) {
      ...
      if(attribute.through === identity && attribute.via === schemaKey)
  ...
  if(vertexA.dominant)
    return { from: vertexA.id, to: vertexB.id };

  if(vertexB.dominant)
    return { from: vertexB.id, to: vertexA.id };
};
