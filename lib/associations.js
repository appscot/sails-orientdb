  ...
  self.connection.find(collectionName, options, function(err, results){
    ...
    criteria.joins.forEach(function(join){
      ...
      if(!join.junctionTable)
        return; //TODO: we should also handle one-to-one and one-to-many joins
      
      var childTableSchema = self.connection.collections[join.child];
      var edgeSides = self.getEdgeSides(join.parent);
      var childSide;
      var parentSide;
      if(edgeSides.out.referencedCollectionTableName === join.child
        && edgeSides.out.junctionTableColumnName === join.parentKey) {
        childSide = edgeSides.out;
        parentSide = edgeSides.in;
      }
      else if(edgeSides.in.referencedCollectionTableName === join.child
        && edgeSides.in.junctionTableColumnName === join.parentKey) {
        childSide = edgeSides.in;
      ...
      results.forEach(function(record){
        ...
        if(!record[parentSide.referencedAttributeEdge])
          return; //it probably has been processed already
        
        record[join.alias] = [];
        if(!_.isArray(record[parentSide.referencedAttributeEdge]))
          record[join.alias].push(record[parentSide.referencedAttributeEdge][parentSide.edgeOppositeEnd]);
