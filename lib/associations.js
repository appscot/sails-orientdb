 ...
 * @api private
 */
Associations.prototype.getFetchPlan = function getFetchPlan(collectionName, criteria) {
  var self = this;
  ...
  Object.keys(parentSchema).forEach(function(attributeKey){
    ...
    var associatedCollectionTablename = self.connection.collections[associatedCollection].tableName || associatedCollection;
...
Associations.prototype.fetchPlanJoin = function fetchPlanJoin(collectionName, criteria, cb) {
  ...
  self.connection.find(collectionName, options, function(err, results){
    ...
    //var expandedResults = self.expandResults(results);  // expandedResults contain circular refs
    ...
    results.forEach(function(record){
      ...
      // utils.cleanOrientAttributes(record, parentSchema);  // experimental
 ...
 * ATTENTION: this may (and probably will) cause circular references
 * take care when iterating over the returned object
 * 
...
Associations.prototype.expandResults = function expandResults(resultset) {
  ...
  var recordMap = utils.reduceNestedObjects(clonedResults, function(accumulator, obj, key){
      if(obj.id)
        accumulator[obj.id] = _.cloneDeep(obj); // ugly but avoids circular references when converting to json
      if(obj['@rid'])
        console.log('WARNING  fetchPlanJoin: object ' + obj['@rid'] + ' still has @rid!');
      return accumulator;
    }, {});
