...
Document.prototype.serializeValues = function serializeValues(values) {
  ...
  Object.keys(values).forEach(function(key) {
    ...
    else if (typeof values[key] === 'object' && (self.schema[schemaKey].model || self.schema[schemaKey].collection)) {
      var associations = this.nestedAssociations || {};
      associations[key] = {
        values : values[key],
        collection : self.schema[schemaKey].model || self.schema[schemaKey].collection
      };
      delete values[key];
    }
