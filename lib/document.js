  ...
  Object.keys(values).forEach(function(key) {
    ...
    if (foreignKey && utils.matchRecordId(values[key])) {
      ...
      if (_.isArray(values[key])) {
        ...
        self.addNestedAssociation(returnResult, key, reducedValues.nestedAssociations, targetCollectionName);
      } 
      else if (_.isPlainObject(values[key])) {
        // Not sure if this scenario is possible
        self.addNestedAssociation(returnResult, key, values[key], targetCollectionName);
        delete values[key];
      }
    }
