...
Edge.prototype.find = function find(criteria, cb) {
  ...
  var edge = self.connection.associations.getEdge(self.identity, criteria.where);
...
Edge.prototype.insert = function insert(values, cb) {
  ...
  var edge = self.connection.associations.getEdge(self.identity, values);
  
  if (edge) {
    // Create edge
    values['@class'] = self.identity;
